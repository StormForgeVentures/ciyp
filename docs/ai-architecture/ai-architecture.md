...
| `embed` | Asymmetric embedding — **`input_type` set explicitly per call**: `document` at index, `query` at retrieval | voyage | `voyage-3.5` @ 1024-dim | Substitution vs canon (Cohere) — ADR-007. Wrong input type = Must-fix; EL-OS ports `embedForIndex`/`embedForQuery` enforce it. |
