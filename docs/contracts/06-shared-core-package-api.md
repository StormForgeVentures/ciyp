...
Defines the surface the two published packages expose. The member UI consumes **`@ciyp/shared`** (types)
and **`@ciyp/ui-tokens`** (design tokens); the engine consumes those **plus** the workspace-internal
`agents`/`prompts` (never published). This contract pins what crosses the repo boundary.

## `@ciyp/shared` — exported surface
...
## `@ciyp/ui-tokens` — exported surface
...
|---|---|---|
| `ciyp-template` (UI) | `@ciyp/shared`, `@ciyp/ui-tokens` | **exact** versions (no ranges) |
| `ciyp-platform` (engine) | `@ciyp/shared`, `@ciyp/ui-tokens` (via `workspace:*`) + internal `agents`/`prompts` | workspace-internal |

## Constraints for downstream

- `@ciyp/shared` is the **only** definition of the 6 contracts. No repo redefines a contract type locally
  (drift-by-construction is the failure we prevent — ADR-004).
- The UI imports **only** `@ciyp/shared` + `@ciyp/ui-tokens`. Importing `agents`/`prompts` in the UI is a
